...
{{--                                        <th>Status</th>--}}
                                    ...
                                    @forelse($list as $key => $data)
                                        <tr>
                                            ...
                                            <td>{{ucfirst($data->user->first_name ?? '') ." ". ucfirst($data->user->last_name ?? '')}}</td>
                                            <td>{{ucfirst($data->contact->userByMobileNumber->first_name ?? 'No name') ." ". ucfirst($data->contact->userByMobileNumber->last_name ?? '')}}</td>
                                            <td>{{$data->id}}</td>
                                            <td>{{$data->templateData->title ?? 'No template name'}}</td>
...
{{--                                            </td>--}}
                                            <td>
                                                <a class="btn waves-effect waves-light btn-grd-info" href="{{route('admin.document.view',['id'=>base64_encode($data->id), 'page'=>request('page')])}}" style="padding: 4px;" target="_blank">
